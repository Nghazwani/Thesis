...
Before splitting the data into two groups for training and testing the ML model, the data need to be prepared to get better results from training. There are two types of data, one with 80 Pile up (meaning 80 collisions/events occur simultaneously within one proton-proton bunch crossing %source)  
another with no pile. Since the No PU will be used in training the ML model it will be further divided into two different categories Full readout and Zero suppression (explain difference %source).   
